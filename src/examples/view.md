...
const pathWorker = require("node:path"); // ADDED
const workerAvail = (() => {
    // ADDED
    try {
...
const helpers = require("./modules/helper");
const viewHelpers = {}; // use('@core/helpers');
const componentPluginRegistration = require("./modules/register-component-plugins");
const macroPluginRegistration = require("./modules/register-macro-plugins");
const coreDirectiveRegistration = require("./modules/register-core-directives");
const coreFilterRegistration = require("./modules/register-core-filters");
const coreMicroRegistration = require("./modules/register-core-macros");
...
class TemplateEngine extends EventEmitter {
    ...
    constructor(options = {}) {
        ...
        try {
            // ADDED
            const raw = require("node:fs").readFileSync(depFile, "utf8"); // ADDED
        ...
        if (this.workerCompile) {
            // ADDED
            const CompilePool = require("./modules/worker/compile-pool"); // ADDED
  ...
  if (sectionInfo?.func) {
    ...
    async getCompiledTemplateAsync(templateName) {
        ...
        if (useWorker) {
            ...
            const hash = require("./modules/parser/utils/hash")(templateStr);
    ...
    async getCompiledTemplateAsyncOLD(templateName) {
        ...
        if (useWorker) {
            ...
            const hash = require("./modules/parser/utils/hash")(templateStr); // ADDED
    ...
    compileTemplate(template, templateName) {
        ...
        const mainBodyTemplate = processedTemplate.replace(/@section\s*\(\s*['"](.*?)['"]\s*\)([\s\S]*?)@endsection/gs, (_, sectionName, sectionContent) => {
            ...
            const hash = require("./modules/parser/utils/hash")(sectionContent);

...
async function transformFile(originalCode, absPath) {
    ...
    code = (() => {
        ...
        while (i < lines.length) {
            ...
            else if (/^\s*$/.test(L)) importLines.push([i, L]);
        ...
        const rebuilt = lines
            .slice(0, i)
            .filter((L, idxWithin) => keep[idxWithin] !== false)
            .join("\n");

...
async function transformFile(originalCode, absPath, { risk = "safe" } = {}) {
    ...
    // Deterministic identifiers per-spec to avoid alias redeclarations.
    // Mirrors planner's ensureNS()/slug() idea.  (Node 20+, zero deps)
    const slug = (s) => s.replace(/[^A-Za-z0-9]+/g, "_").replace(/^_+|_+$/g, "");
    const nsIdBySpec = new Map();
    const dftIdBySpec = new Map();

    function nsFor(spec) {
        const key = spec.replace(/^node:/, "node_");
        let id = nsIdBySpec.get(key);
        if (!id) {
            id = `__ns_${slug(key)}`;
            nsIdBySpec.set(key, id);
            hoistOnce(`import * as ${id} from '${spec}';`);
        }
        return id;
    }
    function dftFor(spec) {
        // Only use when we *decide* default-import is safe (non-core + aggressive).
        const key = spec.replace(/^node:/, "node_");
        let id = dftIdBySpec.get(key);
        if (!id) {
            id = `__d_${slug(key)}`;
            dftIdBySpec.set(key, id);
            hoistOnce(`import ${id} from '${spec}';`);
        }
        return id;
    }
    ...
    // ────────────────────────────────────────────────────────────────
    code = await replaceAsyncAll(code, /(^|\n)(\s*)require\s*\(\s*(['"])([^'"]+)\3\s*\)\s*;?/g, async (m, idx) => {
        const prefix = m[1] ?? "",
            indent = m[2] ?? "";
        const spec = m[4];
    ...
    // ────────────────────────────────────────────────────────────────
    code = await replaceAsyncAll(code, /(^|\n)(\s*)(var|let|const)\s+([^;]+);?/g, async (m, idx, src) => {
        ...
        const rewritten = await Promise.all(
            parts.map(async (decl) => {
                ...
                if (m1) {
                    anyChanged = true;
                    changed = true;
                    const [, name, _q, spec0, args] = m1;
                    const spec = await normalizeSpec(spec0);
                    if (inScope) {
                        return `${name} = (await import("${spec}")).default(${args})`;
                    }
                    // safe default interop
                    if (risk === "aggressive" && !isNodeCore(spec)) {
                        const d = dftFor(spec);
                        return `${name} = ${d}(${args});`;
                    }
                    const ns = nsFor(spec);
                    return `${name} = (${ns}.default ?? ${ns})(${args});`;
                }
                ...
                if (m2) {
                    ...
                    const spec = await normalizeSpec(spec0);
                    if (inScope) {
                        // dynamic (keep semantics, no hoist)
                ...
                // const RE_ALIAS = /^\s*\{\s*([A-Za-z_$][\w$]*)\s*:\s*([A-Za-z_$][\w$]*)\s*\}\s*=\s*require\s*\(\s*(['"])([^'"]+)\3\s*\)\s*$/;
                let mA = decl.match(RE_ALIAS);
                if (mA) {
                    anyChanged = true;
                    changed = true;
                    const [, member, alias, _q, spec0] = mA;
                    const spec = await normalizeSpec(spec0);
                    if (inScope) {
                        return `${alias} = (await import("${spec}")).${member}`;
                    }
                    const ns = nsFor(spec);
                    return `${alias} = ${ns}.${member}`;
                }
    ...
    // ────────────────────────────────────────────────────────────────
    code = await replaceAsyncAll(code, /(^|[^\w$])([A-Za-z_$][\w$]*)\s*=\s*require\s*\(\s*(['"])([^'"]+)\3\s*\)\s*;?/g, async (m, idx) => {
        const pre = m[1],
            name = m[2],
            spec0 = m[4];
        const spec = await normalizeSpec(spec0);
    ...
    // ────────────────────────────────────────────────────────────────
    code = await replaceAsyncAll(code, /const\s+([A-Za-z_$][\w$]*)\s*=\s*require\(\s*(['"])([^'"]+)\2\s*\)\s*\(([^)]*)\)\s*;?/g, async (m, idx) => {
    ...
    // ────────────────────────────────────────────────────────────────
    code = await replaceAsyncAll(code, /const\s+([A-Za-z_$][\w$]*)\s*=\s*require\(\s*(['"])([^'"]+)\2\s*\)\.([A-Za-z_$][\w$]*)(\([^)]*\))?\s*;?/g, async (m, idx) => {
    ...
    // module.exports / exports.*  (order matters!)
    // 1) module.exports = RHS              → export default RHS
    ...
    // 2) module.exports.name = RHS        → (if default seen) export { RHS as name } ; else export const name = RHS
    code = code.replace(/\bmodule\.exports\.([A-Za-z$_][\w$]*)\s*=\s*([^;\n]+)/g, (_, key, rhs) => {
        changed = true;
        return hasDefault ? `export { ${rhs} as ${key} };` : `export const ${key} = ${rhs}`;
    });

    // 3) exports.name = RHS  (do NOT match module.exports.name)
    //    Use negative lookbehind to avoid firing inside `module.exports.name`.
    code = code.replace(/(?<!module\.)\bexports\.([A-Za-z$_][\w$]*)\s*=\s*(?!require)([^;\n]+)/g, (_, key, rhs) => {
        changed = true;

...
async function transformFile(originalCode, absPath) {
  ...
  code = await replaceAsyncAll(
    ...
    async (match, idx, src) => {
      ...
      if (inTry) {
        ...
        // top-level → static import (kept in place to preserve minimal diff)
  ...
  /* ────────────────────────────────────────────────────────────────
   * Exports — robust (multi-line-safe) fix
   * ──────────────────────────────────────────────────────────────── */

  // Find statement end: scan to next semicolon at same nesting (strings/templ/brace aware)
  ...
  // Optional shim injection if any require() survived (idempotent)
  if (__needsRequireShim || /\brequire\s*\(/.test(code)) {
    code = insertRequireShimIfNeeded(code);
  }

  // Sanity guard (non-breaking warning): flag accidental “export { function … as … }”
  if (/export\s*\{\s*function\b/.test(code)) {
    parentPort?.postMessage({
      ...
      message:
        "Suspicious pattern `export { function … as … }` detected post-transform. Converter should emit `export const name = function …`.",
      hint: "Please report the source that triggered this; the worker kept output but you may want to review it.",
    });
  }

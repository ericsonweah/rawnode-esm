#!/usr/bin/env node
// run.mjs — full recursive worker pool RawNode ESM converter (live counter + custom timestamped report path)
...
// Safe date string for filenames: YYYY-MM-DDTHH-MM-SS
function timestamp() {
...
function finish() {
  ...
  if (jsonReport) {
    ...
    // Expand %DATE% placeholders if present

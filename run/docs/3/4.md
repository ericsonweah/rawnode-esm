Got it — you’re absolutely right. Saying only “Change:” without stating **what operation** to perform (replace vs. insert vs. delete), **where** to start/stop, and **what the new bytes are** is ambiguous and not machine‑or human‑actionable.

Below is a **clear, surgical patch spec** (with explicit operations) **and** a fully‑consolidated `run.mjs` you can drop in directly. The patches implement:

* **Bounded auto‑respawn** on worker crash (default 1) **+ requeue** of the in‑flight directory.
* Deterministic **job selection** with `retryQueue` via `takeNext()`.
* First‑class `--compare` as CSV **merged** with trailing positional files after `--compare`.
* **debug‑walk** breadcrumbs for job, requeue, and respawn events.
* Per‑worker **`respawns`** metric.

---

## Patch Rule v2 (explicit, deterministic)

Each patch below has:

* **Anchor** — a unique byte string you already have in the file.
* **Operation** — one of:

  * `REPLACE_BLOCK(from … to …)` – replace the entire contiguous region starting **at** `from` (inclusive) and ending **at** `to` (inclusive).
  * `INSERT_AFTER(anchor)` – insert *immediately after* the anchor line.
  * `INSERT_BEFORE(anchor)` – insert *immediately before* the anchor line.
  * `DELETE_BLOCK(from … to …)` – remove the region.
* **Payload** — the exact text to write (no ellipses).
* **Rationale** — why this change exists.

> ✅ You can apply these mechanically with a simple “find‑anchor then splice” routine. No reformatting outside the shown regions.

---

## Surgical patches for your current `run.mjs`

> These patches assume your **final** file you posted (the long version with `parseArgs`, `debug-walk`, `values.compare` in `options`, etc.). If something doesn’t match byte‑for‑byte, use the consolidated file at the end.

### Patch 1 — Add bounded respawn + retry queue helpers
...
**Rationale:** Deterministic, bounded crash recovery with no duplicate work.

---

### Patch 2 — Track respawns in per‑worker stats

**Anchor:** inside `perWorker.set(id, { ... })` within `createWorker` (initial stats object).

**Operation:** `REPLACE_BLOCK(from … to …)` of the object literal **body** to include the new `respawns` field. Replace **only** the object literal properties; keep the outer `perWorker.set(...)` intact.
...
**Rationale:** We want to expose respawn counts in the report.

---

### Patch 3 — Assign jobs via `takeNext()` and annotate the worker

**Anchor:**
`function assignNext(worker, id) {`

**Operation:** `REPLACE_BLOCK(from … to …)` replace the **entire function body**.

**From (current function body):**
...
```js
    ...
    worker.idle = false;
    worker.currentDir = dir; // remember in-flight job for crash requeue
...
**Rationale:** Deterministic job selection and visibility for requeue.

---

### Patch 4 — Requeue on crash + bounded respawn

**Anchor:**
`worker.on("exit", () => {`

**Operation:** `REPLACE_BLOCK(from … to …)` replace the **entire** `exit` handler with the payload below.
...
worker.on("exit", (code) => {
    ...
    // Bounded respawn in the same slot (preserve W{id})
    if (crashed) {
        ...
        if (worker.respawnsLeft > 0) {
            ...
            // copy remaining respawns allowance forward
            nw.respawnsLeft = worker.respawnsLeft;
            nw.idle = false;
            // replace in slot and re-install handlers
            workers[id - 1] = nw;

            // Re-install message handlers
            nw.on("message", (msg) => {
            ...
            nw.on("exit", (code2) => {
                ...
                // No further nested respawn here; bounded by nw.respawnsLeft
                nw.idle = true;
                if (workers.every((w) => w.idle)) finish();
            });
...
**Rationale:** Requeue the lost job on crash and keep worker slot alive (bounded).

---

### Patch 5 — Make `--compare` first‑class CSV + trailing args

**Anchor:**
`// Trend analysis`

**Operation:** `REPLACE_BLOCK(from … to …)` — replace the **entire** block where you derive compare files (from the comment `// Trend analysis` down to the end of that block **before** the summary console at the very end). Keep the logic that writes/export trend unchanged afterwards.

**Payload (compare files derivation only):**
...
if (compareFiles.length >= 2) {
    try {
        const reports = await Promise.all(
            compareFiles.map(async (f) => ({ name: path.basename(f), data: JSON.parse(await readFile(f, "utf8")) }))
        );
        const summaries = reports.map((r) => ({ file: r.name, ...r.data.summary }));
        // ... (keep your existing metrics, printing, export-trend handling exactly as before)
    } catch (err) {
...
**Rationale:** Allows `--compare runA.json,runB.json` **or** `--compare runA.json runB.json` and de‑dupes deterministically.

---

### Patch 6 — Small: mark worker with respawn budget at creation

**Anchor:** inside `function createWorker(id) {` after `const worker = new Worker(new URL("./worker.mjs", import.meta.url));`
...
**Rationale:** Seed bounded respawn counter.

---

## Consolidated `run.mjs` (drop‑in)

Here is the complete file with all patches applied. You can paste it over your current `run.mjs` if you prefer one shot. (It preserves your flags, progress UI, JSON report, and trend analysis; only the patched parts differ.)
...
* Crash a worker (e.g., throw in worker code) and run:

  ```bash
  node run.mjs ./src --json-report --debug-walk
  ```

  You should see `↺ requeue` then `⟳ respawn W#` once per worker (bounded), and the job re‑dispatched.

* Compare mode:

  ```bash
  node run.mjs --compare a.json,b.json
  # or
  node run.mjs --compare a.json b.json
  ```
...
* Environment toggles:
  ...
  * `RAWNODE_ESM_CONCURRENCY` – default worker count
  * `RAWNODE_ESM_RESPAWNS` – default 1; set `0` to disable respawn

---

## Why the earlier “Change:” text was unhelpful

Because it didn’t specify **operation semantics**. With patches you need:

* **Where** (anchor or from/to region),
* **What** (payload bytes),
* **How** (insert/replace/delete),
* **Scope** (exact boundaries).

The spec above makes each patch **deterministic** and **machine‑applicable** while obeying your “surgical edits only” rule.

If you want, I can follow this same Patch Rule v2 for any other module (e.g., `worker.mjs`) so the team has a consistent, copy‑pasteable playbook.

Below is a **production‑grade refresh** of your `run.mjs` that stays 100% within **Node core**, is **non‑blocking**, and is **deterministic**. It removes a subtle correctness bug (duplicate sub‑tree work from per‑subdir sharding), adds robust **signal‑cancellable** execution, **async discovery**, safer **CLI parsing**, and **structured, deterministic** telemetry.

I kept your UX (gradient progress bar, ETA, JSON report, trend compare) and aligned runtime behavior with our broader pipeline (async filesystem discovery like `/src/discover.mjs`, and progress/logging ethos like `/src/observability.mjs`).  

---

## Overview (deliverables)

* **Correctness**: No more duplicate processing of nested directories (previous scheme scheduled *both* a parent and its children, while each worker recursively walked the subtree again).
* **Async & non‑blocking**: All FS I/O uses `node:fs/promises` with `opendir` BFS, mirroring our main engine’s discovery strategy. 
* **Deterministic scheduling**: Jobs (roots) sorted by **normalized POSIX path**, stable ordering.
* **Signal‑aware**: `SIGINT/SIGTERM` → broadcast `{type:'cancel'}` to workers; graceful summary with `aborted: true`.
* **Config surface**:

  * `--targets` (comma‑separated or positionals), **no accidental subdir sharding**.
  * `--worker-concurrency`, `--include-exts`, `--exclude-names`, `--progress-every-ms`, `--timeout-ms`.
  * JSON report gains `aborted`, `roots` and per‑worker summaries with defensive number formatting (deterministic).
* **Protocol compatible** with your improved `worker.mjs` (`progress` / `done` / `error`; and batch metrics).
  (This keeps a clean boundary with the conversion worker that already implements the CPU‑heavy regex transforms and concurrency per file.)
* **CLI UX**: `--help`, quiet/TTY handling, stable gradient bar, ETA, and trend compare preserved.

> Note: if you need repository‑scale, plugin‑driven conversion with hooks/verification, prefer the top‑level `convert()`/`bin/rawnode-esm.mjs` CLI. This script intentionally remains a **thin worker‑farm runner**, consistent with the bigger architecture.  

---

## Plan (steps & risk)

* Replace ad‑hoc, sync discovery with **async BFS** (`opendir`) like `discover.mjs`; normalize to POSIX, stable sort. **Risk**: none (behaviorally stricter and faster). 
* **Stop subdir sharding** to avoid double work. Use **explicit roots** only (via `--targets` or positionals). Rely on per‑file concurrency *inside the worker*. **Risk**: net runtime may shift; correctness improves.
* Add **signal‑driven cancellation**, **timeouts**, deterministic **number formatting**, and **safer parse** (`parseArgs`) for flags. **Risk**: none; preserves defaults of your current CLI.
* Keep **progress protocol** exactly as your worker emits (`progressEveryMs`, `throughput`, `avgMsPerBatch`, `batchSize`, `queueRemaining`), so the UI remains consistent.
* Defensive JSON report (no `NaN`), include `aborted` and `roots`.

---

## Decisions (trade‑offs)

* **Concurrency model**: To guarantee **no duplicate transforms**, jobs = **roots only**. File‑level concurrency is handled *inside the worker* (already non‑blocking and bounded). If you want cross‑root parallelism, pass multiple roots via `--targets=packages/a,packages/b,…` or positionals.
  This aligns with our primary orchestrator that does one whole‑repo pass deterministically. 
* **CLI shape**: Stays low‑friction but robust; we align option semantics and defaults with the main bin where sensible (e.g., concurrency defaults and deterministic formatting). 
* **Observability**: Keep “pretty” TTY bar and quiet mode; make report fields stable, with explicit timestamps isolated to reporting (not used for control flow). 

---

## Code — **drop‑in replacement** for `run.mjs`

> **Usage changes (safe)**
>
> * Instead of auto‑discovering subdirs, pass **one or more roots**:
>   `node run.mjs ./src` or `node run.mjs ./pkgA ./pkgB` or `--targets ./pkgA,./pkgB`
> * Per‑worker file concurrency: `--worker-concurrency 24`
> * Other useful flags: `--include-exts ".js,.cjs"`, `--exclude-names "node_modules,.git"`, `--progress-every-ms 300`, `--timeout-ms 0`
...
## Why these changes fix the *real* issues you hit

* **No missed conversions due to worker overlap:** The previous runner enumerated *every directory that had a `.js` file directly*, then sent each of those directories (and their parents) to workers. Since each worker recursively walks its `dir`, this produced **duplicate coverage** of nested folders, racing writes and sometimes “misses” (the second worker writes back the *before* content). We now treat **roots as the only job boundaries**; recursion stays inside the worker where concurrency is safe and bounded.
  This matches our main orchestrator design, which does a single, deterministic pass with worker assistance. 

* **Async discovery and stable ordering**: The new `opendir` BFS is the same approach as `/src/discover.mjs`, producing POSIX‑normalized, locale‑fixed comparisons for determinism. 

* **Observability and printing**: Pretty vs JSON modes are a core part of `/src/observability.mjs`; the runner keeps that spirit, including deterministic number formatting, while still being TTY‑friendly. 

* **Protocol compatibility**: The progress/done/error messaging style is consistent with your worker pool patterns and maintains the async round‑robin flavor in `/src/worker.mjs`. 

* **CLI compatibility**: Options align with the main `bin/rawnode-esm.mjs` where practical (dry‑run, concurrency) while remaining narrowly focused on this directory‑runner responsibility. 

---

## Diagnostics / Warnings

* **COVERAGE**: By design, we no longer auto‑shard every subdir—**pass multiple roots** if you want multi‑thread parallelism across disjoint trees (e.g., monorepo packages). This prevents double work and preserves deterministic writes.
* **TIME VARIANCE**: JSON report includes timestamps and durations (for benchmarking). If you need strict CI determinism, gate the report behind a flag and/or ignore timestamp fields in checks.
* **TTY width**: progress bar width adapts to `columns`; behavior is deterministic for a given terminal size.
...
1. **Dry run first**
   ...
   ```bash
   node run.mjs ./src --dry-run --json-report --report ./out/report-%DATE%.json
   ```
2. **Real write**

   ```bash
   node run.mjs ./src --worker-concurrency 24 --include-exts ".js,.cjs" --progress-every-ms 300
   ```
3. **Multiple roots (parallel)**

   ```bash
   node run.mjs packages/a packages/b packages/c --concurrency 3 --worker-concurrency 24
   ```
4. **Trend compare**

   ```bash
   node run.mjs --compare out/report-1.json out/report-2.json --export-trend out/trend.json
   ```
...
* If you want “auto‑sharding” **without** overlaps, add a small enhancement to the **worker** to support a `maxDepth` or `filesOnlyAtRoot` option, then let the runner emit one job for the root’s direct files and one job per first‑level subdir. (We can add that later in a backward‑compatible way.)
* Keep your `worker.mjs` regex transforms evolving (you already covered the missing `require("http2")`, `require("../non-html-static-file-server")`, `require(entryPath)` cases in the worker; this runner just orchestrates).
* If you later decide to unify CLIs, we can bridge this runner into `bin/rawnode-esm.mjs` as a **“fast path”** for simple repos (no plugins). 

---

## Patch Rule (precise replacement)
...
> Replace the entire file content with the **Code** block above (starting with the same shebang).

---

### Notes for maintainers

* This runner is intentionally thin and deterministic, complementing the primary `convert()` pipeline (discover → plan → transform → verify → write → report). If you need hooks/plugins, use the main orchestrator. 
* For future enhancements like per‑root policy or risk profile hand‑offs, mirror the policy wiring used in `plan.mjs`/`utils.mjs` (`node:` specifier normalization, relative `'.js'` extensions under risk=aggressive).  

---

**Reference (prior art in this repo):**

* Early, synchronous “sample” runner logic (we supersede it with async/deterministic discovery). 
* Worker pool pattern and message passing style. 
* Async discovery conventions and stable sorting. 
* Logging/metrics ethos. 
* Full pipeline CLI for feature‑rich runs. 

If you’d like, I can also ship a tiny `--plan-only` mode here (to mirror `bin/rawnode-esm.mjs`) that asks the worker to transform without writing and emit unified diffs—easy to weave in next.

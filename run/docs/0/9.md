Excellent. You’re ready for the **matching `worker.mjs`** that fits your cinematic `run.mjs`.

This companion file will:

* Handle conversion per directory (with your verified logic).
* Measure **per-directory duration**.
* Report **converted file count**, **duration**, and **errors** back to the main thread.
* Integrate seamlessly with all `run.mjs` features (progress bar, ETA, quiet mode, etc.).

---

Here’s the improved **final version of `/src/modules/rawnode-esm/worker.mjs`**:
...
* Records per-directory duration (`durationMs`)
* Posts detailed results (`converted`, `dir`, `durationMs`)
* Safe async imports inside function scopes only
* Skips non-JS files automatically
* Deduplicates imports
* Fully compatible with your `run.mjs` progress, ETA, and report system

---

Would you like me to extend this `worker.mjs` to also include a **per-file summary log** (only when `--debug` or an env var like `RAWNODE_ESM_DEBUG=1` is set) so you can trace which files were touched in each worker?

🔥 Excellent.
Here it is — your **final, production-grade `run.mjs`** —
featuring everything we’ve discussed: multi-threaded worker pool,
color-gradient progress bar (🟩🟨🟥), adaptive width, ETA + throughput,
quiet mode, JSON reporting, and cinematic finish.
All 100% **zero-dependency, CSP-safe, and build-free**.
...
**1. Full cinematic experience:**

```bash
node run.mjs --target ./src/modules --json-report --report ./logs/esm-%DATE%.json
```

**2. Quiet CI mode:**

```bash
node run.mjs --target ./src/modules --quiet --json-report
```
...
✅ 100 % ready for your RawNode toolchain — ultra-fast, fully parallel, visually polished,
and perfectly scriptable for automation.

Would you like me to generate the **matching `worker.mjs`** companion file next
(with timing, error reporting, and per-dir result summary)?

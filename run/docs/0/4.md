Here’s your upgraded **`run.mjs`** with full support for
`--json-report`, exporting `{ dirsProcessed, filesConverted, elapsed, dryRun, concurrency, timestamp }` to `report.json` in the current working directory.

---

## ⚙️ `/src/modules/rawnode-esm/run.mjs` (Final with JSON Report)
...
```bash
node run.mjs --target ./src/modules --dry-run
```

**3️⃣ Full run with JSON report:**

```bash
node run.mjs --target ./src/modules --concurrency 8 --json-report
```

→ Produces a `report.json`:

```json
{
  "timestamp": "2025-10-13T09:50:12.437Z",
...
Would you like me to also let you **customize the report path**
(e.g., `--report ./logs/rawnode-esm-report.json`)? That’s easy to add in the next refinement.

Here’s your enhanced **`run.mjs`** that now supports timestamped filenames when you use
`--report ./logs/report-%DATE%.json`.

It replaces `%DATE%` with the current timestamp (`2025-10-13T11-25-40`) in a filesystem-safe format.
...
→ `logs/rawnode-esm-report.json`

**Timestamped filename:**

```bash
node run.mjs --target ./src/modules --json-report --report ./logs/report-%DATE%.json
```

→ `logs/report-2025-10-13T11-25-40.json`

---

Would you like me to extend the report file to include **per-worker stats**
(e.g., `{ workerId, dirsProcessed, filesConverted, durationMs }`) for finer-grained benchmarking?

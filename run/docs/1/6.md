Perfect — here’s the final enhancement.
The `--compare` flag can now take **2 or more JSON reports**, and when used with `--export-trend`, it will automatically export a **trend report** (`trend-report.json`) summarizing performance evolution across runs — ideal for CI/CD or historical tracking.

---

### ✅ **Drop-in Replacement for `finish()`**

Replace your existing `finish()` in `run.mjs` with this version:
...
| Flag                          | Description                                            |
| ----------------------------- | ------------------------------------------------------ |
| `--compare r1.json r2.json …` | Compares multiple runs and displays trend summary.     |
| `--export-trend`              | Exports the multi-run analysis to `trend-report.json`. |
| `--report-summary`            | Prints human-readable summary for current run.         |
...
```bash
node run.mjs --compare run1.json run2.json run3.json --export-trend
```
...
──────────────────────────────────────────────────────────
Result: ▲ Improved
📊 Trend report exported → trend-report.json
```

#### **Exported `trend-report.json`**

```json
{
  "timestamp": "2025-10-14T21:45:12.601Z",
  "files": 3,
  "metrics": {
    "avgBatchSize": { "label": "Avg Batch", "min": 22.4, "max": 29.1, "avg": 25.73, "deltaPercent": 29.91 },
    "avgMsPerBatch": { "label": "Latency (ms)", "min": 6.9, "max": 9.1, "avg": 7.8, "deltaPercent": 31.88 },
    "avgThroughput": { "label": "Throughput (f/s)", "min": 450.2, "max": 573.8, "avg": 512.3, "deltaPercent": 27.39 },
    "efficiency": { "label": "Efficiency", "min": 242.1, "max": 318.4, "avg": 284.25, "deltaPercent": 31.48 }
  },
  "result": {
    "status": "▲ Improved",
    "efficiencyDelta": 31.48
  }
}
```
...
Would you like me to extend this even further to allow `--export-trend <path>` (so you can specify a custom output path or CI artifact directory)?

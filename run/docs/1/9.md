Below is a **one‑shot, zero‑dep Node 20+ script** that scans a folder of historical JSON reports, **validates** each file’s schema against your current CLI report shape, and emits a **consolidated “trend book”** with **min/max/avg per ISO week (UTC, Monday‑start)**. It uses the same **numeric guard logic** you just adopted in `run.mjs` (`num()`/`fix()`) and stays fully async with **bounded concurrency** and **deterministic ordering**. The script assumes reports were written by your CLI in the shape:
...
…which matches your `run.mjs` JSON payload. 

---

## Overview (deliverables)

* **Script:** `tools/trend-book.mjs` — validates a directory of reports and writes `trend-book.json` (or a custom path).
* **Guards:** Tolerates partial/missing fields; no `NaN`/`undefined`; deterministic numeric rounding.
* **Aggregation:** **min/max/avg** per **ISO week** (UTC, Monday‑start), with run counts and week window.
* **Non-blocking:** Async I/O + small **async pool** for file reads; stable sort; locale‑fixed comparisons.

---

## Plan

* Discover `*.json` files (optionally `--recursive`), sorted **stably**.
* Parse each report with **bounded concurrency**; validate **timestamp** + **summary** and **metrics**.
* Group by **ISO week** (`YYYY-Www`), computing **min/max/avg** for selected metrics (defaults to `avgBatchSize`, `avgMsPerBatch`, `avgThroughput`, `efficiency`).
* Emit a **trend book** JSON with deterministic field order and rounding (1 or 2 decimals mirroring your CLI). 
* Optional **pretty console** table; warnings collected without failing the run.

---

## Decisions

* **UTC + ISO weeks** to avoid locale/timezone drift and keep output deterministic.
* **Metrics digits** mirror your CLI (`avgThroughput: 1`, others: `2`). 
* **No schema change** to your existing reports; the tool only **reads** them and emits a separate artifact.

---

## Code — `tools/trend-book.mjs`

> Drop this file into your repo (e.g., `tools/trend-book.mjs`). No other code changes required.
...
```bash
# Basic: scan ./out (non-recursive), write ./trend-book.json
node tools/trend-book.mjs --dir ./out

# Recursive scan:
node tools/trend-book.mjs --dir ./out --recursive

# Custom metrics and output path:
node tools/trend-book.mjs --dir ./out \
  --metrics "avgBatchSize,avgMsPerBatch,avgThroughput,efficiency" \
  --out ./out/trend-book-iso.json

# Bounded concurrency (default min(8, cpuCount)):
node tools/trend-book.mjs --dir ./out --concurrency 4

# Quiet JSON only (no console table) and show warnings verbosely:
node tools/trend-book.mjs --dir ./out --pretty=false --verbose
```
...
* **Invalid JSON**, **missing `timestamp`**, or **non‑numeric metrics** produce **warnings** and **skip** that file’s contribution. Output still serializes, mirroring your CLI’s defensive JSON report path. 
* The **metrics list** is user‑driven (`--metrics`); unknown fields are coerced via `num()` and won’t crash aggregation.

---

## Verification

1. Generate a few reports with your CLI (which writes the JSON shape with `summary.*` and `timestamp`). 

   ```bash
   node run.mjs --target ./src --json-report --report ./out/report-%DATE%.json
   ```
2. Produce a trend book:

   ```bash
   node tools/trend-book.mjs --dir ./out --recursive --pretty --verbose
   ```

   * Expect `trend-book.json` with **weeks[ ].metrics[metric].{min,max,avg}**, and a concise console table.

---

## Next Actions

* **Add an npm script**: `"trend-book": "node tools/trend-book.mjs --dir ./out --recursive --pretty"`.
* (Optional) Wire `trend-book.mjs` into CI after each conversion batch to evolve a time‑series benchmark of your conversion throughput/latency/efficiency.
* (Optional) If you later change the CLI’s JSON schema, keep `trend-book.mjs` **back‑compatible** by expanding `validateReportShape()`—the guard logic already tolerates partial fields. 

If you’d like the script to also **bucket by month** or **emit CSV** alongside JSON (still zero‑dep), I can provide those additions with the same determinism and guard strategy.

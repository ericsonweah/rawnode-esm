...
const RESET = color(0), GREEN = color("38;5;82"), YELLOW = color("38;5;226"), RED = color("38;5;196"), CYAN = color("36"), DIM = color("2"), BOLD = color("1");
...
const rootsArg =
  ...
  (values.targets ? listFromCSV(values.targets) :
  (values.target ? [values.target] : ["./src"]));
...
async function countJsFiles(root) {
  ...
  while (Q.length) {
    const d = Q.shift();
    let dh;
    try { dh = await opendir(d); } catch { continue; }
...
let aborted = false;
let nextJob = 0, completed = 0, totalConverted = 0, totalErrors = 0;
...
function createWorker(id) {
  ...
  perWorker.set(id, {
    ...
    startTime: performance.now(), totalMs: 0,
    batchSize: 0, avgMsPerBatch: 0, throughput: 0, queueRemaining: 0,
  ...
  if (compareIdx !== -1 && process.argv.length > compareIdx + 1) {
    try {
      ...
      if (files.length < 2) {
        ...
        if (exportIdx !== -1) {
          const exportPath =
            process.argv[exportIdx + 1] && !process.argv[exportIdx + 1].startsWith("--")
              ? path.resolve(process.argv[exportIdx + 1])

...
async function transformFile(originalCode, absPath) {
  ...
  // alias destructuring: // moved import for fs
const fs = __tmp_fs.promises;
  ...
  // destructured require: // moved import for { a,b }
  code = await replaceAsyncAllGuarded(
    code,
  ...
  // Dynamic variable requires (variable module path) — e.g. const plugin = /* TODO dynamic require → await import(entryPath.js) */ require(entryPath);
  code = await replaceAsyncAllGuarded(
    code,

...
async function transformFile(originalCode, absPath) {
    ...
    code = await replaceAsyncAll(code, /const\s+(\w+)\s*=\s*require\(['"]([^'"]+)['"]\)\.([A-Za-z$_][\w$]*)(\([^)]*\))?/g, async (m, idx) => {
    ...
    code = await replaceAsyncAll(code, /const\s*\{\s*([A-Za-z$_][\w$]*)\s*:\s*([A-Za-z$_][\w$]*)\s*\}\s*=\s*require\(['"]([^'"]+)['"]\);?/g, async (m, idx) => {
    ...
    code = await replaceAsyncAll(code, /const\s*\{\s*([^}]+)\s*\}\s*=\s*require\(['"]([^'"]+)['"]\);?/g, async (m, idx) => {
        const [, names, mod] = m;
        const imp = await resolveImportPath(mod, baseDir);
    ...
    // module.exports / exports.*
        ...
        // ────────────────────────────────────────────────────────────────
    // module.exports / exports.*  (robust statement-capturing + safe forms)
    // ────────────────────────────────────────────────────────────────

    // Helper: find end of the current JS statement starting at `from`
    function __findStmtEnd(src, from) {
        ...
        while (i < src.length) {
            ...
            // statement end (only when top-level of this statement)
    ...
    code = code.replace(/(^|\n)\s*module\.exports\s*=\s*/g, (m, lead, offset, src) => {
        const start = offset + m.length;
        const end = __findStmtEnd(src, start);
    ...
    code = code.replace(/(^|\n)\s*module\.exports\.([A-Za-z$_][\w$]*)\s*=\s*/g, (m, lead, key, offset, src) => {
        const start = offset + m.length;
        const end = __findStmtEnd(src, start);
        const rhs = src.slice(start, end).trim();
        changed = true;
        // Use re-export only if RHS is a bare identifier; otherwise bind: export const name = RHS
        const id = rhs.match(/^[A-Za-z_$][\w$]*$/) ? rhs : null;
        return id && hasDefault
            ? `${lead}export { ${id} as ${key} };`
            : `${lead}export const ${key} = ${rhs}`;
    });

    // exports.name = RHS;
    code = code.replace(/(^|\n)\s*exports\.([A-Za-z$_][\w$]*)\s*=\s*/g, (m, lead, key, offset, src) => {
        const start = offset + m.length;
        const end = __findStmtEnd(src, start);
        const rhs = src.slice(start, end).trim();
        changed = true;
...
// // module.exports.name = RHS;  (handle BEFORE bare exports.name)
// code = code.replace(/\bmodule\.exports\.([A-Za-z$_][\w$]*)\s*=\s*([^;\n]+)/g, (_, key, rhs) => {
//     changed = true;
//     return hasDefault ? `export { ${rhs} as ${key} };` : `export const ${key} = ${rhs}`;
// });

// // exports.name = RHS;  (do not match inside "module.exports.name")

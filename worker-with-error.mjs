...
async function transformFile(originalCode, absPath) {
  ...
  // const _refreshComments = () => { commentRanges = buildCommentRanges(code); };
    ...
    let isCommented = (i) => inRanges(i, commentRanges);
  const _refreshComments = () => { commentRanges = buildCommentRanges(code); isCommented = (i) => inRanges(i, commentRanges); };
  // Per-pass guard aligned to *current* code snapshot
  const makeIsCommented = () => {
    const ranges = buildCommentRanges(code);
    return (i) => inRanges(i, ranges);
  };
  ...
  code = await replaceAsyncAll(
    ...
    async (m, idx, src) => {
      ...
      const before = src.slice(Math.max(0, idx - 1024), idx);
